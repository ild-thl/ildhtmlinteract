...
require_once($CFG->dirroot . '/mod/ildhtmlinteract/backup/moodle2/backup_ildhtmlinteract_stepslib.php');

/**
 * Provides the steps to perform one complete backup of the Resource instance
 */
class backup_ildhtmlinteract_activity_task extends backup_activity_task {
    ...
    protected function define_my_steps() {
        $this->add_step(new backup_ildhtmlinteract_activity_structure_step('ildhtmlinteract_structure', 'ildhtmlinteract.xml'));
    }
    ...
    static public function encode_content_links($content) {
        ...
        $search="/(".$base."\/mod\/ildhtmlinteract\/index.php\?id\=)([0-9]+)/";
        $content= preg_replace($search, '$@RESOURCEINDEX*$2@$', $content);

        // Link to resource view by moduleid.
        $search = "/(".$base."\/mod\/ildhtmlinteract\/view.php\?id\=)([0-9]+)/";
        // Link to resource view by recordid
        $search2 = "/(".$base."\/mod\/ildhtmlinteract\/view.php\?r\=)([0-9]+)/";

        // Check whether there are contents in the resource old table.
        /*
        if (static::$resourceoldexists === null) {
            static::$resourceoldexists = $DB->record_exists('ildhtmlinteract_old', array());
        }
        */
        ...
        if (static::$resourceoldexists) {
            ...
            if ($result) {
                ...
                $oldrecs = $DB->get_records_select('ildhtmlinteract_old', "cmid $insql", $params, '', 'cmid, newmodule');
            ...
            // Resource ID links.
            list($insql, $params) = $DB->get_in_or_equal($matches[2]);
            $oldrecs = $DB->get_records_select('ildhtmlinteract_old', "oldid $insql", $params, '', 'oldid, cmid, newmodule');
